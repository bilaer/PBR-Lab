...
// Set this to 0 to avoid double flip on V as texture loader already fliped image vertically
...
std::shared_ptr<Mesh> GlbLoader::LoadMesh(const tinygltf::Model& model,
    ...
    if (!hasNormal) {
        // simple smooth normals from triangles
        auto addFace = [&](uint32_t i0, uint32_t i1, uint32_t i2) {
            const glm::vec3& p0 = mesh->GetVertices()[i0].position; // we don't have GetVertices here; compute on local array
        };

        // Recompute on our local array:
        auto& vtx = vertices;
...
std::shared_ptr<PBRMaterial> GlbLoader::LoadMaterial(const tinygltf::Model& model,
    ...
    // Base color (RGB from baseColorFactor)
